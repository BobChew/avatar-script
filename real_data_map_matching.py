...
if __name__ == "__main__":
    if len(sys.argv) != 12:
        ...
        acc_file = open(output_prefix + "_" + selection_strategy + "_" + sys.argv[10] + ".json", "a")
        # Write all the result table into file
        ini_acc_str = json.dumps(ini_acc_table)
        scan_percent_str = json.dumps(scan_percent)
        selection_acc_str = json.dumps(selection_acc)
        hit_rate_str = json.dumps(hit_rate)
        active_ratio_str = json.dumps(active_ratio)
        acc_str = json.dumps(acc_table)
        acc_file.write(ini_acc_str + "\n")
        acc_file.write(scan_percent_str + "\n")
        acc_file.write(selection_acc_str + "\n")
        acc_file.write(hit_rate_str + "\n")
        acc_file.write(active_ratio_str + "\n")
